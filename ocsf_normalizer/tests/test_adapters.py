...
def expected_status_id(vendor, raw):
    ...
    if vendor == "ecs":
        ...
        outcome = event.get("outcome") or raw.get("event.outcome", "unknown")
        return 1 if outcome == "success" else (2 if outcome == "failure" else 99)
...
def expected_src_ip(vendor, raw):
    ...
    if vendor == "ecs":
        ...
        return source.get("ip") or raw.get("source.ip")
    if vendor == "qradar":
...
def expected_dst_ip(vendor, raw):
    ...
    if vendor == "logscale":
        return raw.get("endpoint_ip") or raw.get("dst_ip")
    return None


def expected_severity_id(vendor, raw):
    if vendor == "qradar":
        magnitude = raw.get("magnitude") or raw.get("severity")
        try:
            mag = int(magnitude)
        ...
        return 5
    if vendor == "logscale":
        level = str(raw.get("loglevel") or raw.get("severity") or "info").lower()
        mapping = {
...
def expected_activity_id(vendor, raw):
    if vendor == "qradar":
        return 2 if "logout" in str(raw.get("action", "")).lower() else 1
    if vendor == "logscale":
        event_type = raw.get("event_type") or raw.get("action") or ""
        return 2 if "logout" in str(event_type).lower() else 1
    return 1


def expected_time_ms(vendor, raw):
    if vendor == "qradar":
        return int(raw.get("starttime") or raw.get("devicetime"))
    if vendor == "logscale":
        ts = raw.get("@timestamp")
        if isinstance(ts, (int, float)):
            return int(ts)
    elif vendor == "sentinel":
        ts = raw.get("EventStartTime") or raw.get("TimeGenerated")
    elif vendor == "ecs":
        ts = raw.get("@timestamp")
    else:
        return None
...
def test_empty_payload_does_not_crash(vendor, validator):
    ...
    assert isinstance(ocsf, OCSFAuthenticationEvent)
    is_valid, errors = validator.validate_event(ocsf.model_dump())
    assert is_valid, f"{vendor} empty payload failed: {errors}"
